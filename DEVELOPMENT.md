...
AltAltTab の開発者向け情報です。ユーザー向けの使い方は README.md を参照してください。
...
- `app`: リリースビルドし、`build/AltAltTab.app` を組み立てて署名する。
- `run`: `app` を実行し、既存プロセスを止めてから起動する。
- `install`: `app` を実行し、`/Applications/AltAltTab.app` にコピーして起動する（日常利用向け）。
...
`Sources/AltAltTab/` 配下、17個の Swift ファイルの役割です。
...
ログはすべて stderr に出力されます（`logToStderr`、`[AltAltTab]` プレフィックス付き）。`log stream` で追うより、ビルド後のバイナリを Terminal から直接実行したほうが手早く確認できます。

```sh
build/AltAltTab.app/Contents/MacOS/AltAltTab
```
