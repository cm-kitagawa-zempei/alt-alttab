...
`.github/workflows/ci.yml` が push（main）・PR・週次（月曜朝 JST）・手動実行で `swift build -c release` → `swift test` → `make app` を実行します。週次実行は、誰も push しなくても macOS ランナーイメージや Swift ツールチェインの更新でビルドが壊れていないかを検知するためのものです。ランナーは `macos-26` を固定使用しており、新しいイメージが来たら手動でバンプします。
